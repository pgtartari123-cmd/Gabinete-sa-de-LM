/* Módulo de múltiplas demandas por cidadão — UX estável */
(function(){
  ...
  function salvar(){
    ...
    const visivel=document.querySelector('.tab[style*="display: block"],.tab[style*="display:block"]')?.id;
    if(visivel==='pessoas'){if(typeof render==='function')render();}
    else if(visivel==='demandas'){if(typeof renderDemandas==='function')renderDemandas();}
  ...
  if(!document.getElementById('multiDemandasStyle')){const s=document.createElement('style');s.id='multiDemandasStyle';s.textContent='.subdemanda-protocolo{display:block;margin-top:4px;color:#3730a3;font-size:11px;font-weight:800}.gd-protocolo-inline{display:inline-block;margin:-3px 0 8px;padding:4px 8px;border-radius:8px;background:#eef2ff;color:#3730a3;font-size:11px;font-weight:800}';document.head.appendChild(s)}
