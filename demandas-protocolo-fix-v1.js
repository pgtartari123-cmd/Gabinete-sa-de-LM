/* GABINETE LM — proteção contra conflito de protocolo
   Não apaga demandas. Se o lote de demandas for rejeitado por protocolo duplicado,
   ele reenvia uma a uma; somente a demanda que colidir é reenviada sem o protocolo,
   preservando todo o restante da demanda e permitindo que a sincronização continue.
*/
(function(){'use strict';
  ...
  async function retryDemandas(url,opt,rows){
    ...
    for(const row of rows){
      const primeiro=Object.assign({},opt,{headers,body:JSON.stringify([row])});
      const r=await originalFetch(url,primeiro);
  ...
  window.fetch=async function(url,opt={}){
    if(opt.__gabineteProtocoloRetry)return originalFetch(url,opt);
    ...
    return retryDemandas(url,Object.assign({},opt,{__gabineteProtocoloRetry:true}),rows);
  };
})();
