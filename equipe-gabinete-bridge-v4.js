...
function openCreateSeparated(){
  ...
  document.body.appendChild(m); m.querySelector('#eqnewx').onclick=()=>m.remove();
  m.querySelector('form').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target);const body={nome:String(f.get('nome')||'').trim(),usuario:String(f.get('usuario')||'').trim().toLowerCase(),cargo:String(f.get('cargo')||''),senha:String(f.get('senha')||'')};const out=m.querySelector('#eqsepmsg');if(!body.nome||!/^[a-z0-9][a-z0-9._-]{2,29}$/.test(body.usuario)||body.senha.length<6){out.className='eqmsg err';out.textContent='Confira nome, usuário e senha (mínimo 6).';return}try{await fn('criar-membro',body);out.className='eqmsg ok';out.textContent='Membro criado com sucesso!';e.target.reset()}catch(x){out.className='eqmsg err';out.textContent='Erro: '+x.message}};
}
function openSeparated(){
  if(!admin()){alert('Apenas o administrador pode gerenciar a equipe.');return} style(); document.getElementById('eqBridgeMgr')?.remove();
  const m=document.createElement('div');m.id='eqBridgeMgr';m.innerHTML='<div class="eqbm"><div class="eqbh"><div><small>ADMINISTRAÇÃO • EQUIPE</small><h2>👥 Membros com acesso</h2><p>Veja e gerencie quem pode entrar no Gabinete LM.</p></div><button id="eqbx" type="button">✕</button></div><button id="eqopennew" style="display:block;width:100%;margin:18px 0;padding:15px;border:0;border-radius:13px;background:#4f46e5;color:#fff;font-weight:800;font-size:17px">➕ Cadastrar novo membro</button><h3>👤 Membros cadastrados</h3><div id="eqBridgeMsg"></div><div id="eqBridgeList">Carregando membros...</div><p style="color:#64748b;font-size:12px;margin-top:16px">🔐 A senha fica no Supabase Auth.</p></div>';
